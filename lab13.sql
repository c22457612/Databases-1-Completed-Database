...
-- lab 13 demonstration -- QUERY 25 ONWARDS
--Queries Demonstrating Entity Integrity:
-- Show Users table
SELECT * FROM Users;

-- Show 2 attributes ensuring entity integrity
...
-- Show 2 attributes ensuring entity integrity
SELECT User_id, Photo_id FROM Likes LIMIT 2;


--Queries Demonstrating Referential Integrity:
-- Show Photos table
SELECT * FROM Photos;

-- Show Foreign Key References (User_id in Photos)
SELECT User_id FROM Photos LIMIT 5;

-- Show Foreign Key References (Photo_id in Likes)
SELECT Photo_id FROM Likes LIMIT 5;

-- Show Users table
SELECT * FROM Users;

-- Show Username Length Constraint
SELECT username FROM Users WHERE LENGTH(username) > 10;

-- Show Username Ending with 'n'
SELECT username FROM Users WHERE username LIKE '%n';

-- Show Photos table
SELECT * FROM Photos;

-- Show Photos Published Before a Certain Date
SELECT * FROM Photos WHERE publication_date < '2020-07-19';


--Queries with JOIN and Aggregate Function:
SELECT Users.username 
FROM Users
JOIN Likes ON Users.User_id = Likes.User_id 
WHERE Likes.Photo_id = '2';

SELECT Users.username, COUNT(Photos.Photo_id) AS photo_count
FROM Users
LEFT JOIN Photos ON Users.User_id = Photos.User_id
